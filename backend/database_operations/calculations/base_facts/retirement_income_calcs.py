...
class RetirementIncomeCalculator:
    ...
    def calculate_income_amount(
        ...
        if not is_active:
            return IncomeCalculationResult(
                ...
                metadata=self._generate_calculation_metadata(income, year, current_age, Decimal('0'))
            )
        ...
        if income.apply_inflation:
            ...
            adjusted_amount = apply_annual_inflation(base_amount, inflation_rate, years_from_start)
        ...
        return IncomeCalculationResult(
            ...
            is_active=True,
            included_in_nest_egg=income.include_in_nest_egg,
            metadata=self._generate_calculation_metadata(income, year, current_age, inflation_adjustment)
        )
    ...
    def calculate_multiple_income_streams(
        ...
        return [
            self.calculate_income_amount(income, year, inflation_rate, plan_start_year)
    ...
    def aggregate_by_source(
        ...
        for result in results:
            if result.is_active:
                ...
                totals[source] = totals.get(source, Decimal('0')) + result.adjusted_amount
        return totals
    ...
    def calculate_total_income(
        ...
        return sum(
            r.adjusted_amount for r in results
    ...
    def calculate_total_inflation_impact(
        ...
        return sum(
            r.inflation_adjustment for r in results
            if r.is_active
        )
